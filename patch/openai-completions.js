                ...
                if (choice.delta) {
                    ...
                    for (const field of reasoningFields) {
                        if (choice.delta[field] !== null &&
                            ...
                            // DeepSeek V4 may return empty reasoning_content that must be
...
export function convertMessages(model, context, compat) {
    ...
    for (let i = 0; i < transformedMessages.length; i++) {
        ...
        if (msg.role === "user") {
            ...
            // Keep thinking blocks with signatures even if empty (DeepSeek V4 needs
            // reasoning_content passed back even when empty in tool call chains).
            const nonEmptyThinkingBlocks = thinkingBlocks.filter((b) => (b.thinking && b.thinking.trim().length > 0) || b.thinkingSignature);
            ...
            // DeepSeek V4 requires reasoning_content to be echoed back on ALL assistant
